...
const LOGIN_CONTAINER = `openclaw-grok-login-${appId()}`;
...
function loginPtyCommand(): string {
  const args = [
    ...
    OPENCLAW_IMAGE,
    '-c',
    'socat TCP4-LISTEN:56122,fork,reuseaddr TCP4:127.0.0.1:56121 & exec openclaw models auth login --provider xai --method oauth'
  ];
...
async function grokAuthed(): Promise<boolean> {
  const { out } = await dockerCapture(
    openclawArgs('models', 'auth', 'list', '--provider', 'xai'),
    PROBE_TIMEOUT_MS
  );
...
export async function GET() {
  ...
  if (env.get('OPENCLAW_ENABLE_GROK')?.value !== '1') return json({ needed: false });
  if (!existsSync(join(STATE_DIR, 'openclaw.json'))) return json({ needed: false });
  return json({ needed: !(await grokAuthed()) });
}
  ...
  const stream = new ReadableStream({
    start(controller) {
      ...
      const poll = setInterval(async () => {
        ...
        checking = true;
        try {
          if (await grokAuthed()) settle(true);
        } finally {
          checking = false;
        }
      }, 3000);
      ...
      c.on('close', async (code) => {
        if (settled) return;
        settle(code === 0 || (await grokAuthed()), code ?? 1);
      });
    },
    cancel() {
      child?.kill();
