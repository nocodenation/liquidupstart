// Classification of .env keys and section behavior markers.
//
// The primary mechanism is the marker suffix in .env.example section titles
// (e.g. "2. SERVICE SECRETS & CREDENTIALS - AUTOGENERATED ONCE IF VALUES ARE
// EMPTY"); key-pattern matching below only picks the input widget and the
// rebuild-needed classification.
...
// Changing any of these requires rebuilding images (section "IMAGE BUILD
// CONFIGURATION" knobs consumed by build.sh, plus the OpenClaw build switch).
...
// One-liners shown in a section's header so a collapsed card still tells the
// user what's inside. Matched against the full banner title (keyword-based,
// so renumbering sections in .env.example doesn't break it); sections without
// an entry fall back to the first sentence of their leading doc paragraph.
