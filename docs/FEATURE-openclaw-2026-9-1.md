...
| | | |
|---|---|---|
| **R1** | `gateway.auth.identityScopes` is written by `config/scripts/start/openclaw.sh` | The recovery exists only while that key is in the configuration, and the start script rewrites `volumes/_openclaw/openclaw.json` on every run. Today's grant is a hand edit and will not survive the next `start.sh`. The identity must come from the **same source** as nginx's `X-Forwarded-User`, not be typed twice |
