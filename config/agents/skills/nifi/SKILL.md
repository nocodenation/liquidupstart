---
name: nifi
description: Apache NiFi flow design rules, construction patterns, visual layout standards, and custom processor / NAR packaging for this environment. TRIGGER when user designs or refactors a NiFi flow, lays out processors/funnels/connections, configures Controller Services, or builds and deploys a custom processor or NAR. For raw API mechanics (auth token, CRUD calls, ingress URLs) use the **nifi-api** skill instead.
---

# NiFi Development Rules

These rules govern all Apache NiFi development in this environment. Enforce them without exception.
...
sibling **nifi-api** skill covers the **API transport** (proxy + `Host:` header, bearer
token, ingress URLs, SSL Context Service properties, links you give the user). When a
rule here needs an API call, follow the connection and auth conventions from **nifi-api**
...
- **ALWAYS use the NiFi REST API directly** (curl, or Python `requests`)

### 2.2 API Configuration — use the nifi-api skill
There is **one** NiFi instance in this environment, reached **only** through the nginx
proxy. The full connection contract — endpoint, `Host:` header, self-signed cert, and
bearer-token auth — lives in the **nifi-api** skill; follow it exactly and do not invent
...
- **Routing header**: `-H "Host: nifi.localhost:${SYSTEM_HTTPS_PORT}"` — the
...
- **Auth**: acquire a Bearer token via `POST /nifi-api/access/token` using
  `$NIFI_USERNAME` / `$NIFI_PASSWORD` (already injected — never ask the user). Send it as
  `Authorization: Bearer <token>` on every call. Tokens last 12 h; on `401`, regenerate.

```bash
HTTPS_PORT=$(echo $SYSTEM_HTTPS_PORT)
NIFI_TOKEN=$(curl -sk -X POST https://proxy:$HTTPS_PORT/nifi-api/access/token \
  -H "Host: nifi.localhost:$HTTPS_PORT" \
...
also goes through the proxy with a `Host:` dynamic property — see the **nifi-api** skill's
*Connecting NiFi to other services* section. Never point a processor at a bare container
...
1. **GET** the component's property descriptors from the NiFi API
...
`nifi.localhost` and its ingress subdomains), mounted read-only at `/certs`. Unlike
...
using the exact property values from the **nifi-api** skill's *SSL Context Service*
section:
...
| Key Password (`key-password`) | value of `$NIFI_KEYSTORE_PASSWORD` |
...
`$NIFI_KEYSTORE_PASSWORD` is injected into the environment — read it with
`echo $NIFI_KEYSTORE_PASSWORD`, never ask the user. The native API property for the key
password is `key-password`, **not** `Key Password`. Enable the service after creating it.

### 3.7 HTTP Context Map Per Ingress
Every `HandleHttpRequest` processor and its associated `HandleHttpResponse` processors MUST share a **dedicated named HTTP Context Map** (`StandardHttpContextMap`). Name it after the ingress it serves — here that means the ingress **port** (e.g., `"ingress-8900"`), since ingresses are addressed as `https://{port}.nifi.localhost:HTTPS_PORT` (ports 8900–8999). All `HandleHttpResponse` processors in the same flow must reference the same context map as their `HandleHttpRequest`.
...
When serving static assets via NiFi (e.g., through `RouteOnAttribute` + `UpdateAttribute`), maintain a **single** `mime.type` expression covering ALL served file extensions (`.ico`, `.gif`, `.png`, `.jpg`, `.css`, `.js`, etc.). When adding support for a new file type, update the existing mime.type expression rather than creating a separate route or processor.

### 3.10 User-Data in Filenames — Sanitization
When using user-provided values (e.g., email addresses) in filenames, always sanitize with NiFi EL `replaceAll` to remove or replace characters that are invalid or problematic in file paths. **Double escaping required**: NiFi EL unescapes `\\` → `\` before passing to Java regex, so a literal backslash in the regex needs **4 backslashes** in the stored property value. Example: `${form.email:replaceAll("[/\\\\\\\\]", "_")}` — NiFi EL reduces to `[/\\]`, Java regex matches `/` or `\`.
...
`HandleHttpResponse` processors ALWAYS have a `success` relationship that MUST be connected to a terminal funnel below the processor. Without this connection, the processor is invalid. This is easy to miss because the success FlowFile carries no useful data, but NiFi requires it to be wired.
...
2. Connect the funnel directly to each target processor (NiFi clones the FlowFile automatically)
...
Every custom NiFi processor MUST be registered in the SPI descriptor or it will not appear in the NiFi UI.
...
`/opt/nifi/nifi-current/lib/` before launching NiFi. So the deploy procedure is:

1. Build the NAR(s).
2. Place the `.nar` file(s) in `./volumes/nar_extensions/`.
3. Restart the container: `docker compose restart nifi` (or `up -d --force-recreate nifi`).
...
just a schema reload. (NiFi also supports hot-loading from an autoload directory, but in
...
**Declare parent NAR in `pom.xml`** (version MUST match the NiFi version of the
...
<dependency>
    ...
    <version>2.x.x</version>  <!-- MUST match the running NiFi version -->
    <type>nar</type>
</dependency>
```
...
docker compose logs nifi | grep -i "classnotfound\|ghost"
```
NiFi's own logs are also under the mounted `./volumes/nifi/` repositories and the
container's `logs/` directory.

### 6.6 Python Processors (NiFi 2.x)
This NiFi (2.x) supports **native Python processors** in addition to Java NARs. Drop a
...
**NiFi API Base URL**: `https://proxy:${SYSTEM_HTTPS_PORT}/nifi-api` with
`-H "Host: nifi.localhost:${SYSTEM_HTTPS_PORT}"` and `-k` (self-signed). Bearer-token auth
only — no mTLS. Resolve the port with `echo $SYSTEM_HTTPS_PORT`. See the **nifi-api** skill.

**User-facing links**: NiFi canvas `https://nifi.localhost:HTTPS_PORT/nifi`; HTTP ingress
`https://{port}.nifi.localhost:HTTPS_PORT/<path>` (ports 8900–8999). Never give the user a
...
**Version**: Apache NiFi 2.x (base image `ghcr.io/nocodenation/liquid-nifi:latest`); custom
Java processors target NiFi 2.x / Java 21. Confirm the exact version via `/nifi-api/flow/about`.
