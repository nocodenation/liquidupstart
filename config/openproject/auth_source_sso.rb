...
module AuthSourceSSO
  def find_current_user
    ...
    Rails.logger.debug { "Check is SSO is enabled: #{auth_source_sso_enabled?}" }
  ...
  def perform_header_sso(login, user)
    ...
    # If LDAP did not return a user, try local user lookup/creation based on headers via ProxyAuthSource
    user ||= ProxyAuthSource.find_user(login, headers: request.headers, config: sso_config)
