 ...
 * business logic (unpdf text extraction, js-tiktoken chunking, embedding via
 * the self-hosted OpenAI-compatible endpoint, PostgREST inserts) is unchanged;
 * only the tool wrapper differs: OpenCode's `tool()` is replaced by OpenClaw's
 * `defineToolPlugin` + typebox parameter schema.
 *
 * Reads a PDF (or a folder of PDFs), extracts text page-by-page, chunks at
 * ~400 tokens with 50-token overlap, embeds each chunk via $OPENCODE_EMBEDDING_HOST,
 * and inserts rows into rag_documents / rag_chunks via PostgREST. The chunk
 * embedding column is `vector(4096)`; pgvector's binary-quantization HNSW index
 * handles the 2000-dim HNSW limit at index time, so we store the raw float
 * vector and let the index/queries do the quantization.
 *
