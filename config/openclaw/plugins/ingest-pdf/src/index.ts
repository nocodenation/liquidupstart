    ...
    for (let i = 0; i < total; i += COPILOT_BATCH) {
        ...
        while (true) {
            ...
            if (r.status === 401 || r.status === 403) {
                throw new Error(
                    `copilot gateway HTTP ${r.status}: ${txt}. The github-copilot provider is not authenticated, or the gateway rejected the request. Sign in via the dashboard's GitHub Copilot panel (or set GITHUB_COPILOT_TOKEN), ensure OPENCLAW_ENABLE_COPILOT=1, and restart OpenClaw.`,
                )
            }
